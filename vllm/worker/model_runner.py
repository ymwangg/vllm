...
class ModelRunner:
    ...
    def prepare_draft_input_tensors(
        self,
        seq_group_metadata_list: List[SequenceGroupMetadata],
        ...
        for seq_group_metadata in seq_group_metadata_list:
            ...
            block_table = seq_group_metadata.d_block_tables[seq_id]
    ...
    def draft_step(
        ...
        if not self.is_driver_worker:
            return None
        input_tokens, input_positions, input_metadata = self.prepare_draft_input_tensors(
            seq_group_metadata_list)
        # special faster draft model step
        tp_size = self.parallel_config.draft_model_tp_size
        ...
        with MarkActiveModel(ActiveModel.DRAFT):
            ...
            logits = torch.matmul(hidden_states.squeeze(1),
                                  model.lm_head.weight.t())
            probs = torch.softmax(logits, dim=-1)
            output = torch.argmax(probs, dim=-1)
            token_ids_list = output.tolist()
            probs_list = torch.split(probs, 1, dim=0)

        for seq_group_metadata, token_id, parent_probs in zip(
                seq_group_metadata_list, token_ids_list, probs_list):
            seq_ids = list(seq_group_metadata.seq_data.keys())
            assert len(seq_ids) == 1
            seq_id = seq_ids[0]
            seq_data = seq_group_metadata.seq_data[seq_id]
            seq_data.append_draft_token(token_id, None, parent_probs)
    ...
    def speculate_execute_model(
        ...
        for _ in range(self.speculate_length):
            ...
            #                     d_kv_caches,
            #                     is_draft_model=True)
            self.draft_step(seq_group_metadata_list, d_kv_caches)
